...
function RunJob($id)
{
    ...
    $job = Start-Job -ScriptBlock { 
        ...
        cd $currentDir
        c:\ProgramFiles\java\jdk8_x86\bin\java.exe -Xms1600M '-classpath' $classPath $mainClass $configFile 2>&1 > "$logDir/out$($id).out"
...
function MANETSimple($id)
{
    for ($i = 1; $i -le 10; $i++) # probability
    {
        ...
        CreateConfig $id 0 0 $knPer $prob 0 0 0 $globTime $features $ensembles $devices $duration
